...
export default class User {
  ...
  income = () => {
    const moneyForHouses = reduce(this.houses, (sum, house) => (house.rented ? sum + house.rentMoney : sum), 0);
    return this.job.currentSalary + moneyForHouses;
  }

  timeLeft = () => this.lifeStats.timeLeftStats(this)

  changeSleepTime = (newSleepTime) => this.lifeStats.changeSleepTime(newSleepTime, this)

  changeSelectedFood = (newFood) => this.lifeStats.changeSelectedFood(newFood, this)

  changeSelectedParty = (newParty) => this.lifeStats.changeSelectedParty(newParty, this)
  ...
  moneyAfterYear = () => this.income() + this.lifeStats.moneyAfterYear(this)
    ...
    const userParentsNextYear = () => {
      if (this.parents !== null && user) {
        ...
        if (diedFather !== undefined || diedMother !== undefined) {
          setColor('warning');
          setInfo(<NumberFormat prefix="At least one of your parents have died, inheritance: " displayType="text" value={diedFather !== undefined ? diedFather.money : 0 + diedMother !== undefined ? diedMother.money : 0} thousandSeparator />);
        }
      }
    };
...
export const generateUser = () => {
  ...
  user.sexuality = Number(!user.gender);
  user.generateNames();
  return user;
};
